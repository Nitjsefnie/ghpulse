...
def test_health_error_branch_reports_version(monkeypatch):
    ...
    monkeypatch.setattr(app_mod, "_health_payload", _boom)
...
def test_health_ok_branch_reports_version(monkeypatch):
    """The success payload carries it too.

    /health here builds its body in _health_payload() and wraps it in a
    Response, so this asserts through the real serialisation rather than
    against the dict — a field dropped by the response layer would still
    be present in the dict.
    """
    app_mod = importlib.import_module("backend.app")

    monkeypatch.setattr(
        app_mod, "_health_payload",
        lambda: {"ok": True, "db": True, "version": version.VERSION},
    )
